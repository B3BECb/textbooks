<?php
	...
	*/
	class teacherMainMenu extends user implements IUser  
	{
        ...
        // ---------
        function EditLesson ($lessId, $lessName, $discription, $img)
        {
            try {
				if (empty($lessName)) throw new Exception("Недопустимое название урока.");

				global $mysqli;
                $name = $mysqli->result($mysqli->query("SELECT img FROM lessons WHERE lesson_id = $lessId"), 0);

                $themeId = $mysqli->result($mysqli->query("SELECT theme_id_fk FROM lessons WHERE lesson_id = $lessId"), 0); // Костыль, на время
				
                $dir = "themes/theme_$themeId/lesson_$lessId"; // путь к каталогу загрузок на сервере
				$file = ($name) ? "$dir/$name" : ""; // полный путь к файлу
				...
				if (@$img['tmp_name'])
				{
					...
					// замена = закачать -> удалить 	
					@unlink("themes/theme_$themeId/lesson_$lessId/$oldName");

					if (!($success = move_uploaded_file($img['tmp_name'], $file))) throw new Exception("Ошибка перемещения файла.");
					$mysqli->query("UPDATE lessons SET lessonName = '$lessName', discription = '$discription', img = '".$img['name']."' WHERE lesson_id = $lessId");
				}
				else 
                if (!empty($img))
                {
                    //картинка не заменяется. запрос на апдейт
                    $mysqli->query("UPDATE lessons SET lessonName = '$lessName', discription = '$discription' WHERE lesson_id = $lessId");
                }
                else
                {
                    //полное удаление изображения
                    $mysqli->query("UPDATE lessons SET lessonName = '$lessName', discription = '$discription', img = '' WHERE lesson_id = $lessId");
                    @unlink("themes/theme_$themeId/lesson_$lessId/$name");
                    $name = '';
                }
				...
				//$this->jsOnResponse("{'type':'edit', 'message':'Тема изменена.', 'success':'1', 'themeId':'$themeId', 'themeName':`$themeName`, 'themeDiscription':`$discription`, 'themeIMG':'$file'}");
                $less = new Lesson;
                $less->EditLessonConstruct($lessId, $themeId, $lessName, $discription, $name);
                echo json_encode(array('success' => '1', 'Msg' => 'Урок изменен', 'lessId' => $lessId,'Element' => $less->getElement()));
			}
			catch (Exception $e)
			{
